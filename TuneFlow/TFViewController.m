...
- (void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI {
    if (self.peripheral != peripheral && self.centralManager != nil) {
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    ...
    if ([stringFromData isEqualToString:@"EOM"]) {
        ...
        NSArray *sharedSongs = [self findSharedSongs:externalSongsToTimes];
        self.dataToSend = [NSKeyedArchiver archivedDataWithRootObject:[self getSongTimeDict:sharedSongs]];
        if ([sharedSongs count] > 0){
            ...
            [vc setSharedSongs: (NSArray *)sharedSongs];
...
-(void)switchPeripheralToCentral{
    if(self.peripheralManager != nil){
        ...
        //not sure if central manager needs to be put to nil really....
        [self.centralManager scanForPeripheralsWithServices:@[ [CBUUID UUIDWithString:kServiceUUID] ] options:@{CBCentralManagerScanOptionAllowDuplicatesKey : @YES }];
        NSLog(@"Scanning started");
        //self.centralManager = [[CBCentralManager alloc] initWithDelegate:self queue:nil];
    }
